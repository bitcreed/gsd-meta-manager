...
**Plans**: 8/8 executed, plus 2 UAT gap-closure plans (19-09, 19-10) and 19 security gap-closure plans (19-11 … 19-27 executed, **19-28/19-29 pending — round 11**) across ten audit rounds. **Audit 10 (2026-09-04) says the modelled surface is COMPLETE**: there is no eleventh plane, and what remains is that the tenth plane has fifteen carriers and only nine of them have a control. Round 11 finishes the disposition rather than looking for a new plane — `T-19-116` (`C-10`, the binary every stub and the guard itself exec: one permitted `cp /bin/true` takes layer 2 AND layer 3 and moved a bare remote's `main`, and it had **no threat id, no severity, no deferred-items row and no control letter**), `T-19-118` (`C-05`, the generated `gitconfig`, whose appended `credential.helper = store` makes `git credential fill` return the ambient secret and falsifies `SECTION_ENVELOPE`'s **first** `Guaranteed` clause), `T-19-115` (rule (a) fails open in **seven** directions, not four — tilde, glob and brace clear `Token.literal` without a `$`) and `T-19-117` (the ledger is unbounded work on the guard's critical path: 190 MB → 7.69 s against `GUARD_TIMEOUT_SECS = 5`)
...
- [x] 19-25-PLAN.md — Round 9 rules. **Option (b): an assignment whose key names a value git RE-PARSES as a command line is unbounded and refuses** — option (a) costed by measurement and rejected (a second tokenizer for git's own `split_cmdline`, attacker-reachable recursion at depth 2, and carriers that deliver no readable body). `REPARSED_COMMAND_SECTIONS` beside `INDIRECTION_SECTIONS`, reading the SECTION plus git's one-byte shell rule, asked at the TWO decision regions that already read that word. **T-19-108 closes only AS SCOPED — audit 7's `!`-bodied destructive pair still works, and that is T-19-86.** Three fail-open directions stated and handed to no control; both residues gain a revisit condition and a non-skipping version witness. **T-19-86 and T-19-91 stay open at `high` — `/gsd-secure-phase 19` is NOT cleared**
- [x] 19-26-PLAN.md — Round 10 corpus, RED, and the first round OFF THE ARGV PLANE. T-19-114 is T-19-76's failure mode for the tenth round and the first time the gap is a different KIND of carrier rather than a wider alphabet: every alphabet in nine rounds draws a governed COMMAND LINE, while every control this phase installs has a FILE carrier no command line has to touch. Produces the EXHAUSTIVE carrier enumeration — fifteen live carriers, two stated non-carriers, one covered ENV family — measured row by row, including five planner-derived ones RECORDED not asserted (`<env>/<alias>/gitconfig` under `AR-19-04`, the empty `gh/` dir, `.claude/settings.json`, `settings.json`'s claimed second carrier `settings_json` which has no production caller, and **C-15 `~/.config/gsd-meta-manager/config.json`, re-read on every Bash call, whose unclamped `pr_cap_*` disarms SAFE-06 with no ledger write and no envelope path named — a distinct route from T-19-112 that rule (a) cannot reach**). Re-drives T-19-112's four-call cap reset in ONE persistent root and rebuilds T-19-111's bare-remote legs with a control beside each. Adds a FIFTH named axis `CONTROL_CARRIER_CLASSES` (seven classes) and a NINTH evidence file. **All four of rule (a)'s fail-open directions are pinned PERMITTED so a rule that quietly widened lands red; every repo-side row is RECORDED because 19-27 writes no rule for it**
- [x] 19-27-PLAN.md — Round 10 rules. **Option (d), a SPLIT: the nine carriers under `<envelope>/<alias>/` get an ABSOLUTE + LITERAL + LEXICAL + COMPONENT-WISE operand-path rule raised once at the top of `classify_segments`' existing loop BEFORE the resolution match; the four repo-side carriers get NO rule.** (c) is dead on a single-uid model; (b) tamper-evidence is available for the append-only ledger but costed and DEFERRED (it needs the second carrier T-19-35 says SAFE-06 has not) and is structurally UNAVAILABLE for the stub, since the thing that would detect a replaced `pre-push` is the binary the replacement removed. **T-19-112 and T-19-113 are NARROWED, not closed — four fail-open directions (redirection target, expansion-borne, symlink, relative) stated in four places and handed to no control.** Repairs `SECTION_ENVELOPE`'s falsified `Guaranteed` cap claim and D-09's ceiling narrative, and performs the **T-19-111 attribution correction at all five sites — required regardless of the fix, because a live non-shell bypass filed under T-19-86 is one the user's scoping decision stops counting.** **T-19-86, T-19-91 and T-19-111 stay open at `high` — `/gsd-secure-phase 19` is NOT cleared**
- [x] 19-28-PLAN.md — Round 11 corpus, RED, and it adopts audit 10's finding whole: the modelled surface is complete, so this round finishes the tenth plane rather than looking for an eleventh. Drives `T-19-116` end to end against real git with a control beside every leg and layer 2 measured separately, and `T-19-118`'s `git credential fill` with the generated-file control first. **Registers `C-10` — a threat id, a severity, a deferred-items row and a control letter — so the fifteen-carrier table has a letter for every one.** `T-19-115` is `T-19-76`'s failure mode for the TENTH consecutive round and the first time it is in the cell the previous round just filled: rule (a) decides on `Token.literal` and inherits round 5's whole class list, while not one entry of the axis's seven classes carries a `~`, `*`, `?` or `{a,b}`. Widens the axis from seven classes to ELEVEN, **refines classes 1, 2 and 5** (class 2 `draws_a_redirection_target_carrier` has no literalness guard, so this round's own glob and brace redirection targets collide with the very alphabet that is moving arm), MOVES `CONTROL_CARRIER_REDIRECTION_TARGETS` out of the invariance arm (`19-18`'s `{v}>` blocker, a sixth time), adds a fail-closed BINARY class with EXACT-PATH-not-PREFIX near-miss controls, and adds a **NO-EXPANSION-SPELLING fence** so the axis can prove it draws each metacharacter. **Its sharpest new control is the SEGMENT-COUNT pin**: `split_segments_with_heads` flushes at an operator token, so a redirection-target token in the stream would split `git >/dev/null push --force origin main` and turn round 6's headline refusal into a permit. **Zero `src/` hunks; ends deliberately RED**
- [x] 19-29-PLAN.md — Round 11 rules. **The design answer is BOTH, plus a third thing that is neither: widen the PATH SET (the guard's own binary, as an EXACT PATH — its directory is shared, so a prefix would refuse `ls ~/.cargo/bin`), widen what the rule can SEE (pathname REDIRECTION TARGETS), and bound the ledger where neither applies.** Closing direction (i) does **not** require the deletion model to read redirection targets as argv: round 6 answers *which words ARRIVE* and rule (a) asks *what the line NAMES*, `skip_redirection_target` already walks the target's extent with its own quoting rules, and the target rides on the `Segment` exactly the way `Segment::redirection_unresolvable` already travels — **`segment.tokens` byte-for-byte unchanged, `SEPARATORS` unmoved, `is_separator(">")` still false.** The residue is restated as **SEVEN directions over two word classes and two paths, as a CONDITION rather than a list**, handed to no pin, schedule or witness; `T-19-115` gets NO rule, because a tilde needs the environment and a glob needs the filesystem. Repairs `SECTION_ENVELOPE`'s **first** `Guaranteed` clause under a 215-token cap with two tokens of headroom and a pinned phrase inside the clause, and takes a **REQUIRED mechanism control for `T-19-118`** — one injected `credential.helper` pair with an EMPTY value, which reads no command line and so defends every write spelling, verified by `git credential fill` failing closed rather than by `git config --get-all`, which still lists the helper and would report a working control as broken. Bounds the ledger read fail-CLOSED from the DEADLINE rather than the caps (**the plan's one named severable item**), and corrects **`mod.rs:165-168`, D-09's last uncorrected home**. **`T-19-86`, `T-19-91` and `T-19-111` stay open at `high`; `T-19-116`/`T-19-117`/`T-19-118` are NARROWED and whether they close is audit 11's judgement — `/gsd-secure-phase 19` is NOT cleared**
- [x] 19-30-PLAN.md — Round 12 corpus, RED, and it adopts audit 11's framing whole: **the plane IS finished** — no twelfth layer, and round 11's three new mechanisms create no new reachable state — **so this round fixes the cell one CHARACTER over rather than looking for a new plane.** `lexical_absolute_components` requires the WORD to start with `/`, so a path after an `=` is invisible to both halves of the path set while `Token.literal` is TRUE; it is none of the seven declared directions, and audit 11 drove it until a bare remote's `main` moved (`835b6be` → `40ad7f7`) and a fired PR cap reset. **`19-28` OBSERVED this and named its mechanism exactly right; `19-29` carried it forward as "not a defect" — the observation was right and the classification was wrong, and the plan says so.** Decides and MEASURES the boundary: **`/`-anchored substrings of a LITERAL word, never a list of attachment characters** — an `=`-split misses `--opt=a=/p`, `tar -C/p` and `host:/p`, and a character list is D-08's defect one level over. **Pins the cost from BOTH sides** (`--author=`, `--format=%H`, `sed s/x/y/`, a URL, `HEAD:refs/…`, `--git-dir=/tmp/g`, an assignment prefix) with the mechanical reason it is bounded by the PATH SET rather than the split. Widens the axis from eleven classes to TWELVE and **EXTENDS class 7, the COMPLEMENT** — the sixth forced arm split, the second through the complement — and adds a **NO-INTERIOR-PATH fence** requiring a non-`=` attachment. Also measures `T-19-121`'s whole key-shape space and `T-19-120`'s FIFO with the fresh-root control. **Zero `src/` hunks; ends deliberately RED**
- [x] 19-31-PLAN.md — Round 12 rules and honesty repairs. Reads every `/`-anchored substring of a LITERAL word through the **EXISTING** normaliser and the **EXISTING** two comparisons at the **ONE existing reading site** — `i == 0` is today's rule, so the candidate set CONTAINS the old one and no refusal can be lost. **Opens three `src/` files and no more; `hooks.rs`, `mod.rs` and `advisory.rs` are NOT opened, and `SECTION_ENVELOPE`'s 211/215 headroom is unspent.** The helper is placed INSIDE the no-filesystem slice with a fourth positive control naming it. **The residue is restated as a CONDITION with NO count** — audit 11's own recommendation — over what the predicate READS, handed to no pin, schedule or witness. Bounds the ledger by KIND **inside the existing `Ok` arm** so a fresh root's first forge call still falls through. **Corrects both false claims whether or not any rule lands**: `policy.rs:5743`'s `dd if=X of=Y` (measured exit 0) and `cred.rs:420-425`'s "already GOVERNED" — the sixth residue bounded on a layer that does not enforce it — with the claim correction in a DIFFERENT commit from the SEVERABLE `credential.helper` clause. **`T-19-116` is NARROWED FURTHER and stays OPEN at `high`**: its four remaining spellings are the other instances of the condition, none of them an option attachment. **`/gsd-secure-phase 19` is NOT cleared**

- [x] 19-32-PLAN.md — Round 13 corpus, RED. **Audit 12 wrote the sentence this round exists to make writable**: the open set is *not yet* the human-decision items plus the deliberately-unruled residues, because `T-19-122` and `T-19-123` are unmodelled classes with driven harm. Adopts audit 12's diagnosis whole — **the residue condition enumerates the silences of a READING, and a boundary has three ways to be silent** — and restates it over the WORD SET, the READING and the PATH SET. `T-19-122`: a here-string word is in NEITHER word class, because `consume_redirection` records a target only when the operator takes a pathname; it deleted the whole envelope, moved a bare remote's `main` and reset a fired PR cap. `T-19-123`: `rm -rf <root>` is exit 0 while `rm -rf <root>/alpha` is exit 2 — the prefix is one component too deep — **and it is PINNED PERMITTED BY NAME in two places, so this plan names both pins for `19-33`'s cross-fence exception and moves neither.** Decides both boundaries: **a SECOND `Segment` field and a THIRD word class (because widening `pathname_target` turns two fenced pins red and a second field turns neither), and an ANCESTOR clause BOUNDED AT THE ENVELOPE ROOT (because a prefix widened to the root refuses a whole subtree when a user sets `GSD_MM_ENVELOPE_ROOT`)**. Measures `T-19-124`'s quadratic on THREE trees and **derives audit 12's own parenthetical remedy INSUFFICIENT** — a byte floor skips nine candidates of eight thousand. Widens the axis from twelve classes to FOURTEEN, **extending classes 1, 2 and 7** — the seventh forced split, the second through class 2. **Zero `src/` hunks, zero `tests/` deletions; ends deliberately RED**
- [x] 19-33-PLAN.md — Round 13 rules and honesty repairs. A THIRD word class and an ANCESTOR clause at the **ONE existing reading site**; `hooks.rs` is not opened and the pathname/non-pathname split stays byte-identical, read from both sides for different reasons. **The protected path set grows by EXACTLY ONE path and stops at the envelope ROOT**, on the same ownership and AR-19-11 grounds that keep the binary half an EQUALITY — with the residue above the root registered rather than accepted. **Both fenced pins MOVE as a named cross-fence exception with each reasoning REWRITTEN, not deleted.** `T-19-124` (SEVERABLE) bounds the **WORK** rather than the input — linearity in the word's length with a fail-closed ceiling a test DRIVES — and moves no component vector. Four honesty repairs land whether or not any rule does: the disclosed cost says NORMALISES; the refusal's recovery lines become actionable for all three cases; `word_is_exactly`'s doc states that the parent's own deletion removes the binary; `ledger.rs`'s inverted gate reason is corrected; and `cred.rs` gains the alias-body route, **recorded as `T-19-86`'s and NOT folded**. **The three-axis condition is written LAST so it records an observed outcome.** **`T-19-116` is NARROWED and stays OPEN at `high`; `/gsd-secure-phase 19` is NOT cleared; and this plan does NOT write audit 12's finish-line sentence — that is audit 13's judgement**
...
- [x] 19-18-PLAN.md — The corpus, first, and RED, on a SECOND AXIS. T-19-99: all seven `UNREADABLE_CLASSES` are word-ASSEMBLY classes, `grep -rnE '"(git|gh|glab)[^"]*[<>][^"]*"' tests/ src/` returns nothing and neither corpus file holds a backslash-newline, so the corpus is structurally incapable of failing on T-19-97 or T-19-98. Adds `DELETION_CLASSES` **beside** a byte-identical `UNREADABLE_CLASSES` — five degenerate-proof predicates spanning a deleted word, an ATTACHED operator, a multi-character or fd-carrying operator and both positions of a line continuation — with a displacement alphabet spliced where the hole actually is (between the governed program and its decision words; the prefix and trailing positions are already refused and are pinned as controls). Writes audit 5's sixteen T-19-97 and eight T-19-98 reproducers plus seven cells found while planning as a FIFTH evidence file, each measured against the built binary and confirmed under bash shims — the continuation rows from a file whose bytes `od -c` verified — and pins the corpus to fail in BOTH directions, including the over-deletion control `git x2>/tmp/o push --force origin main` (bash gives git `[x2] [push] …`) and the two pre-existing FALSE REFUSALS this round removes. **Zero `src/` hunks; ends deliberately RED**
- [x] 19-19-PLAN.md — The rule, COMPLETED rather than replaced: **the words the guard classifies must be exactly the words the program receives, in the same order.** Round 4's `Token.literal` is one half and audit 5 verified it complete; word SURVIVAL is the other. Design option (a) — `tokenize` MODELS deletion — taken over option (b) on a measured cost: refusing governed commands that carry a deletable token would refuse `git log > out`, `gh pr list 2>/dev/null` and, decisively, `gh pr create --title x > /tmp/o`, which is COUNTED with a ledger line today. Deletion happens in the ONE walk, so a deleted word never becomes a `Token`, every decision index is over the surviving argv, and `first_unreadable_decision_word`'s one closure needs **no second reading site**. `SEPARATORS` is byte-identical and `is_separator(">")` stays false — the doc's reasoning is right and only its unexamined consequence is corrected. The production is `[IO_NUMBER|{varname}] OPERATOR WORD` with `&>` recognised before `&` reaches the separator arm, and anything it cannot complete marks the command UNRESOLVABLE through the arms the exhaustive post-filter already has. `\`+newline is consumed producing no character in the unquoted arm and the double-quote loop, with `literal` deliberately left TRUE because a deletion is not a rewrite. **Net cost is NEGATIVE: two measured over-refusals removed. T-19-86 and T-19-91 stay open at `high` — `/gsd-secure-phase 19` is NOT cleared**

**Wave 15** *(gap closure from audit 6 of `/gsd-secure-phase 19`, 2026-08-29 — audit 6 enumerated bash's transformations one at a time and declared the command-line-to-argv boundary CLOSED with no third shell mechanism, then found the class still open one level past it: having reconstructed argv, the guard must still decide which word in it is the VERB, and it asks a hand-maintained, unpinned list about* git's *grammar and fails open when the list is silent. Six rounds modelled the shell; this one models the callee. Corpus first again, on the seam that worked three times; 19-21 blocked on 19-20)*

- [x] 19-20-PLAN.md — The corpus, first, and RED, on a THIRD AXIS. T-19-101: `UNREADABLE_CLASSES` and `DELETION_CLASSES` are both axes of the SHELL's grammar and `grep -rn "attr-source\|shallow-file\|GIT_GLOBAL_VALUE_OPTS" tests/` returns nothing, so the corpus is structurally incapable of failing on T-19-100. Adds `CALLEE_GRAMMAR_CLASSES` **beside** two byte-identical shell axes — separate-value, value-less, attached-`=`, not-accepted-by-this-git, and the end-of-options markers — with a class-tagged `GIT_GLOBAL_OPTIONS` alphabet spliced where `scan_leading` reads, and a SEPARATE `GIT_GLOBAL_UNKNOWN_OPTIONS` alphabet kept out of the invariance arm because its entries are not verdict-preserving. Writes audit 6's nine T-19-100 rows, T-19-102's false refusal and twelve cells found while planning as a SIXTH evidence file, each measured against the built binary AND against the real `git` binary with a two-sided probe (`git <opt> version` versus `git <opt> XVALUE version`) whose table is recorded — the probe alone shows this git answers `unknown option: --super-prefix` for an entry the constant carries. Pins the corpus to fail in BOTH directions: twelve ordinary invocations at exit 0 with `git --no-pager status` as this axis's `ls {git,svn}-repo`, and `git push --signed no origin <ref>` pinned REFUSED so a fix copied from `git push -h` lands red. Re-expresses the production/comment ratio control as an ABSOLUTE floor on stripped production bytes (measured 25.0936% against a 25% floor — 246 bytes of headroom) so 19-21's comments cannot break a file it may not edit. **Zero `src/` hunks; ends deliberately RED; asserts every derivable row at its POST-fix verdict so the handoff needs no replacement exception**
- [x] 19-21-PLAN.md — The rule, INVERTED for the third time: **a leading `git` option whose grammar the guard cannot establish makes the VERB SLOT UNESTABLISHED, and an unestablished verb is `Refuse` — never the next non-`-` word.** The same fail-closed treatment `resolve_program`'s wrapper axis already has, which audit 6 names as the one arm that never got it. `leading_git_option`'s `(None, 1)` default becomes a third answer; the knowledge required is reduced to ONE BIT per option before it is enumerated, with three knowledge-free structural rules first — an attached `=` is self-contained whatever the option is, `--` ends the options, a non-`-` token ends the scan. One unpinned list becomes two probed constants (`--attr-source` and `--shallow-file` ADDED, `--super-prefix` REMOVED), pinned against the REAL git binary in a TEST — never in the guard, because the guard runs on the agent's critical path and a `git` earlier on `PATH` can lie about its own grammar. T-19-102 closes with a ONE-entry change to `PUSH_VALUE_OPTS` and `push_operands` deliberately does not fail closed, because its failure direction is over-refusal rather than bypass. **Cost measured at ZERO on git 2.43 and one refusal per future global option, pinned from both sides. No second reading site, no new park reason, `hooks.rs` unopened. T-19-86 and T-19-91 stay open at `high` — `/gsd-secure-phase 19` is NOT cleared**
...
**Phase risks**:
  ...
  - ~~**MUST-SPIKE (OQ5)**: podman rootless is entirely unexercised — no podman was installed on any research machine. Volume permissions (`--userns=keep-id`), `--format json` shape (docker NDJSON vs podman array), and general Docker-CLI compatibility are all LOW confidence. Auto-detect is a user-locked hard requirement, so install podman and exercise `bollard::connect_with_podman_defaults()` end to end before this phase's planning finalizes~~ **RESOLVED 2026-09-15** — spiked empirically against podman 5.7.0 rootless + docker 29.1.3 rootful; see `22-SPIKE-OQ5.md`. Headlines: the mount argv is runtime-specific and inverted (`--userns=keep-id` on podman, `--user $UID:$GID` on docker; each is wrong or rejected on the other), the Docker-compat REST API erases the `ps --format json` divergence so no normalization layer is needed, and `bollard::connect_with_podman_defaults()` **silently falls back to the Docker socket** — runtime identity must be read from `version().components`, never inferred from the constructor
...
**Plans**: 4 plans in 3 waves (0/4 executed). Research answered all five routed questions **by measurement on this host** and four came back different from what the artifacts anticipated, so the plan set is shaped around the measurements rather than around the ROADMAP's assumptions. Four conflicts are each visibly resolved: **C-1** — an egress allowlist is enforceable on rootful docker and **not** on rootless podman inside the spike's no-privileged/no-cap-add fence (slirp4netns has no destination filter, `--internal` needs `passt` + `aardvark-dns`, `CAP_NET_ADMIN` is absent), so the phase keeps D-22-14's intent, changes its claim to "restricted where the runtime can enforce one", types the posture so an unenforced restriction is unrepresentable, and **records on the run which one it got**; **C-2** — `Libpod-Api-Version` is unreachable through bollard (`ping()` returns the body only), so `version().components` is the sole identity route and an **absent** components array refuses rather than defaulting to docker; **C-3** — the second compile-error tripwire CONTEXT.md believed existed does **not** (the spawn closure matches on the profile, not the target), so the variant addition produces exactly ONE compile error and the closure's tripwire is added deliberately; **C-4** — a podman API socket comes up with no systemd and no privilege, so the remedy is named (`systemctl --user start podman.socket`) and automating it is **declined**, because a new spawn site would widen the allowlist `tests/spawn_seam_guard.rs` fences. Two findings changed the work itself: the driven CLI's **session history lives inside `CLAUDE_CONFIG_DIR`** under a slug that is the project's absolute path with slashes as dashes (measured), so one named volume carries credentials **and** resume state and the identity path map of D-22-08 is mechanically forced rather than preferred; and envelope option (b) is **false on two independent grounds** (a `git push` to a local path remote needs neither network nor credential, and a missing hooks-path directory was measured pushing with exit 0 and **no warning at all**), so the Phase 19 envelope is made live inside the container by bind-mounting the host binary read-only at its identical absolute path — **in scope, roughly one plan**, because splitting it out would ship a phase whose success criterion 4 is knowingly false with a security boundary as the difference. The `ps --format json` normalization the ROADMAP budgeted for is **deleted, not deferred** (D-22-05): the Docker-compat API returns identical payloads from both runtimes

Plans:
**Wave 1**

- [ ] 22-01-PLAN.md — **Tracer: one containerized run, end to end.** `bollard 0.21.1` with its measured cost recorded in the repo's own idiom (25 newly locked crates, 354 → 379, the declined hand-rolled client named), the `VerifiedRuntime` capability token in the `DrivableProject` register, connect-and-ask detection with an absent components array as a refusal, the spike § F2 mount/uid recipe cited by row, the identity path map, the named credential volume, the envelope's two mounts, the glibc `containers/Dockerfile`, and an `#[ignore]`d arm proving the stream-json duplex survives the container boundary. Also: the phase's whole refusal taxonomy landed interface-first, the spawn-closure tripwire (C-3), and `ExecutionTarget` **promoted** — the `Default` derive removed so no site reaches a target by omission (CTNR-01, CTNR-02, CTNR-03, CTNR-05)

**Wave 2** *(blocked on Wave 1 completion)*

- [ ] 22-02-PLAN.md — **The pre-flight gate and the container's lifecycle.** Five named refusals before anything is created — the socket, the network backend, the image's ability to execute the envelope binary, the credential volume's authentication, the egress posture — each naming its remedy by name, each reached through the runtime API and never a child process, because the seam guard fails in both directions. Carries the D-22-11 correction RESEARCH measured (an unauthenticated config dir emits an ordinary `system/init` the Phase 15 gate ACCEPTS, so D-22-12's refusal is load-bearing), `stop_container` bounded below the two existing graces, and `tests/container_envelope_gap.rs` committing git's measured fail-open as the regression that proves the probe is load-bearing (CTNR-02, CTNR-03, CTNR-05)
- [ ] 22-03-PLAN.md — **Target selection, judged at one boundary.** Per-project on the registry entry (surviving a downgrade through the existing flattened catch-all), per-run on `--target` / `--container-image`, override wins, both judged at `DriveArgs::from_argv` and never by a clap `value_parser`, reaching the detached driver through the one existing spawn seam with no second path. Closes the `AGENT_OVERRIDE_FIELDS` gap deliberately: an image reference is one entrypoint-shaped mistake from the hazard those fields are debug-gated for (CTNR-01, CTNR-04, CTNR-05)

**Wave 3** *(blocked on Wave 2 completion)*

- [ ] 22-04-PLAN.md — **The verified target, the durable record, and the TUI's stop.** The selection becomes a `VerifiedRuntime`-carrying target through the pre-flight or the run **refuses** rather than falling back to the host; the durable `target` string stops being a `Debug` rendering and gets one total label function with the reader accepting **both** spellings (which is what discharges D-22-16's `one-way` rating by design change rather than by re-rating); `container_id` and `egress` land on the run record with the scope word their own census demands; and stop reaches the container through the API before the process-group teardown. Plus the contract test that stops the singular host assumption returning (CTNR-01, CTNR-04, CTNR-05)
...
- [x] 24-01-PLAN.md — Parser: Goal lines, `#### Build phase N` placeholder phases, shipped-milestone facts, sanitised real-roadmap fixtures (wave 1)
- [x] 24-02-PLAN.md — Layout engine: transitive reduction, git-log lanes, milestone bands, per-phase facts and cursor navigation (wave 1)
- [x] 24-03-PLAN.md — Tab renumber: Roadmap first, Pipeline→Phases at 2, PhaseList removed, Archive at an interim ninth slot (wave 1)

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 24-04-PLAN.md — Master/detail widget: list + detail pane, side-by-side ≥100 cols, stacked below (wave 2)
- [x] 24-05-PLAN.md — Roadmap cursor and keys, ProjectState adapter, selection shared with Phases (wave 2)

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 24-06-PLAN.md — Render wiring, PhaseList header content, fixture defect pins, escape probe, legacy graph removed (wave 3)
...
- [x] 25-01-PLAN.md — Core agent-worktree scan (lock-free git), adapter seam, liveness thresholds, AGENT requirement ids (wave 1)

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 25-02-PLAN.md — Claude Code enrichment adapter: metadata, transcript-mtime liveness, worktree-less live agents (wave 2)
- [x] 25-03-PLAN.md — Wave model: plan attribution, done/finished/running/stalled/queued, summary ladder (wave 2)
...
- [x] 25-07-PLAN.md — close CR-01: orphaned agent worktrees age out (`MAX_AGENT_AGE_SECS` → `Ended`), `Finished` alone never switches the Status cell on, running agents choose phase/ladder (WR-01), fixer estimate needs a running fixer; byte-identical dashboard regressions
...
| 19. GITSAFE — Git & Blast-Radius Envelope | 33/33 | In Progress|  |
