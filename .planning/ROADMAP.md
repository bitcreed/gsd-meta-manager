...
- [ ] 19-26-PLAN.md — Round 10 corpus, RED, and the first round OFF THE ARGV PLANE. T-19-114 is T-19-76's failure mode for the tenth round and the first time the gap is a different KIND of carrier rather than a wider alphabet: every alphabet in nine rounds draws a governed COMMAND LINE, while every control this phase installs has a FILE carrier no command line has to touch. Produces the EXHAUSTIVE carrier enumeration — fourteen live carriers, two stated non-carriers, one covered ENV family — measured row by row, including four planner-derived ones RECORDED not asserted (`<env>/<alias>/gitconfig` under `AR-19-04`, the empty `gh/` dir, `.claude/settings.json`, and `settings.json`'s claimed second carrier `settings_json` which has no production caller). Re-drives T-19-112's four-call cap reset in ONE persistent root and rebuilds T-19-111's bare-remote legs with a control beside each. Adds a FIFTH named axis `CONTROL_CARRIER_CLASSES` (seven classes) and a NINTH evidence file. **All four of rule (a)'s fail-open directions are pinned PERMITTED so a rule that quietly widened lands red; every repo-side row is RECORDED because 19-27 writes no rule for it**
