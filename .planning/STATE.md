...
Last activity: 2026-09-26 - Completed quick task 260926-jnf: Mask secret config values in Cfg tab + typed rows for missing gsd-core keys
...
| 260925-x0v | Refuse git linked worktrees at every registration route (auto-discovery, CLI/TUI add, create-project) and prune stale worktree entries on config load | 2026-09-25 | cdd7d94 | [260925-x0v-prevent-git-linked-worktrees-from-being-](./quick/260925-x0v-prevent-git-linked-worktrees-from-being-/) |
| 260926-06g | Linux-only agent-process liveness signal for the Phase 25 running-agents view: a locked agent worktree whose lock-owner pid is dead (missing, start-time mismatch, or cwd outside the project) reads `Ended`; a `codex` process with cwd inside an agent worktree reads `Live`. /proc reading reused from `session_detector.rs` behind a single `process_probe()` seam (non-Linux: always "unknown", no guessing), fed from the existing 5s agents scan. WR-06 partly mitigated on Linux, WR-02..WR-05 untouched. Ran unisolated on the primary checkout | 2026-09-26 | 5391c2f, 95adad4, dfb3b94 | [260926-06g-use-agent-process-cwd-as-a-linux-only-li](./quick/260926-06g-use-agent-process-cwd-as-a-linux-only-li/) |
| 260926-0u3 | Session auto-discovery: a live session whose cwd is inside a git linked worktree (or a subdir of one, or a `.claude/worktrees/agent-*` path when git resolution fails) resolves to the MAIN worktree and auto-registers it once (needs `.planning/`, canonical-path dedupe, usual rules; alias from main dir name; 13 concurrent worktree sessions → one entry). The worktree itself is never registered; manual CLI/TUI add unchanged. Overrides 260925-x0v's [inferred] skip. Ran unisolated on the primary checkout | 2026-09-26 | 1690957, b19fca0, 90681d3 | [260926-0u3-auto-register-main-project-when-a-sessio](./quick/260926-0u3-auto-register-main-project-when-a-sessio/) |
| 260926-16t | Stale HANDOFF no longer shows as Paused (phase behind STATE.md current_phase, or STATE.md last_updated >1h newer than handoff timestamp/mtime; dimmed "Stale HANDOFF ignored" line in detail header, no badge/needs-human/Paused line). One `phase_progress()` (current-milestone roadmap phases, done = executed on disk or ticked if no dir) now feeds dashboard cell, Roadmap header and bands (ttbook 5/6 everywhere, was 4/6 vs 5 of 11). Ran unisolated on the primary checkout | 2026-09-26 | e05931c, 2445c8a, a992770 | [260926-16t-fix-stale-handoff-shown-as-paused-and-in](./quick/260926-16t-fix-stale-handoff-shown-as-paused-and-in/) |
| 260926-1t1 | 4a detail-view navigation model: `DetailFocus` TabBar/Content level (←/→ tabs, ↓/Enter descend with no action — Enter on Sessions no longer resumes on arrival), Esc steps up one level / q leaves, ↑ on first row → tab bar; ←/→ and [/] switch Sessions/Docs sub-tabs (Roadmap [/] unchanged, m hidden alias), digits land in content and remember last sub-tab; bracketed active tab, cyan border + ▸ on focused pane (Backlog), single Sessions border, level-aware footer + help block; `DetailRegions` per-frame rects for 4b waves pane/mouse. Ran unisolated on the primary checkout | 2026-09-26 | c2eae58, e1a704b, be23374, e88be01 | [260926-1t1-4a-detail-view-navigation-model-tab-bar-](./quick/260926-1t1-4a-detail-view-navigation-model-tab-bar-/) |
| 260926-2l4 | 4b Phases-tab Waves pane: waves section moved to top of the Phases right pane as a focusable (→/Enter in, ←/Esc out), scrollable pane (j/k, PgUp/PgDn, g/G); sub-stages collapsed to two lines; plan rows glyph+WORD state, NN-MM id, PLAN.md title, act/est tokens (token table dropped); finished waves fold/merge, current wave expanded and windowed; `e` opens PLAN.md at objective line; per-plan state kept by agents scan; waves.json + plan titles cached in refresh scan (no render I/O); Agents sub-view one-line wave strip + Enter cross-jumps Agents <-> Waves; `DetailRegions` waves pane + row rects for mouse. Ran unisolated on the primary checkout | 2026-09-26 | 067fb8c, b41ba3d, 99782db, f1ba834 | [260926-2l4-4b-phases-tab-waves-pane](./quick/260926-2l4-4b-phases-tab-waves-pane/) |
| 260926-dyf | Mouse support: click top tabs / sub-tabs / list rows (dashboard, Phases, Sessions, Agents, Waves pane) selects + focuses; double-click = Enter (incl. cross-jumps); wheel moves selection in pane under pointer; crossterm capture on at start, always off on exit/panic/editor hand-off; `preferences.mouse` default true; `M` runtime toggle with status message; help/README/GETTING-STARTED/CONFIGURATION updated; 33 new mouse_* tests. Ran unisolated on the primary checkout | 2026-09-26 | 5a1717d, 001c78b, e54ab55, d8740b7 | [260926-dyf-enable-mouse-support-in-the-tui](./quick/260926-dyf-enable-mouse-support-in-the-tui/) |
| 260926-fcp | Clear clippy lints under `cargo clippy --all-targets -- -D warnings` (12 sites / 9 lint classes in src/browser.rs, src/project_creator.rs, tests/envelope_*.rs; no `#[allow]`, no behavior change). Release gate `cargo clippy -- -D warnings` was already green and stays green. Ran unisolated on the primary checkout | 2026-09-26 | 56f5583 | [260926-fcp-clear-pre-existing-clippy-errors-so-the-](./quick/260926-fcp-clear-pre-existing-clippy-errors-so-the-/) |
| 260926-fi9 | v1-era ROADMAP phase lines parsed: plain/bold-with-tail checkbox items, bare `Phase 01: Name (…)` lines inside shipped collapses, spaced-dash headings, decimal ids. Shipped-collapse phases go to a display-only `shipped_phases` list (Roadmap shipped bands; excluded from main list, Start now, Parallel). Dedupe: one row per phase per list, field-merge (checked-if-either, higher plan counts, first non-empty name); main list wins across lists, no cross-list merge (renumbered milestones). Fixture tests/fixtures/roadmap-shapes/. Ran unisolated on the primary checkout | 2026-09-26 | d94f980, e9a4085, c458c7c | [260926-fi9-parse-v1-era-roadmap-md-phase-heading-sh](./quick/260926-fi9-parse-v1-era-roadmap-md-phase-heading-sh/) |
| 260926-gd5 | Doc key tables synced to handlers: README dashboard `n`→`c` (create project), added b/s/Tab/Ctrl+C/arrows rows, `d` = unregister only; detail tab-bar j/Space/k + non-wrapping arrows; new per-tab key table (Roadmap, Phases, Waves pane, Backlog, Git, Queue, Sessions, Agents, Config, Docs sub-tabs); GETTING-STARTED alias-then-path add dialog, `c`, sub-tab arrows; CONFIGURATION eight tabs not ten. Help-overlay/footer-vs-handler findings F1–F8 recorded in SUMMARY (no code change). Doc-only, ran unisolated | 2026-09-26 | 2a83c50, 63b7f70, 0bb3874 | [260926-gd5-fix-readme-getting-started-key-tables-ag](./quick/260926-gd5-fix-readme-getting-started-key-tables-ag/) |
