...
Last activity: 2026-09-23 - Completed quick task 260923-md1: Roadmap tab dependency graph view with milestone boundaries (graph default, `v` toggles box list; 5 commits 8137449..10a0bf3; tests 1 known env failure only). Previously: 2026-09-17 - Completed quick task 260917-r4c: the three-tag publish-failure cycle is closed at the SOURCE rather than one test at a time. The `publish` job ran plain `cargo test`, so fail-fast revealed exactly one environment-dependent failure per tag; `scripts/pre-tag-check.sh` could not catch the class because it validates THIS machine, where the GSD oracle and every other ambient dependency is always present — the local gate was green on precisely what CI was red on. A faithful `ubuntu-latest` lookalike container (git 2.55.0 from `ppa:git-core/ppa`, Node 24, `gh` 2.101.0 from GitHub's own apt repo, rustc stable + the 1.88.0 floor, non-root uid 1000, empty `HOME` with no `~/.claude`) was run under `cargo test --no-fail-fast` with all 45 integration binaries plus lib, bin and doc-tests verified to have executed: **2118 passed, 3 failed, and that is the complete list** — `tests/driver_router_conformance.rs:377` (the oracle, the genuine runner blocker) and `src/envelope/policy.rs:9478`'s two `gh api` grammar pins, the latter two failing only because the first image omitted `gh`, which `ubuntu-latest` ships and which makes both PASS at 2.101.0. The git-version witness passes in the container. **The oracle was PROVIDED, never switched off**: `GSD_META_MANAGER_ALLOW_MISSING_ORACLE` appears nowhere, since setting it makes the conformance run vacuous — green while checking nothing, the fail-open shape WR-07 exists to forbid. New `scripts/install-conformance-oracle.sh` reads `GSD_CORE_SYNCED_VERSION` out of `src/state_reader/config_json.rs` at run time (no baked literal, same staleness argument as the git constant), installs that pinned `@opengsd/gsd-core`, links it onto `Oracle::resolve()`'s first branch, and refuses to exit 0 until a real `query init.manager` answers. The `publish` job gains `actions/setup-node@v4` and that script before `Test`, and `Test` becomes `--no-fail-fast`; the `msrv` job is byte-identical and no new workflow file was added. `--container` on `pre-tag-check.sh` builds `scripts/pre-tag-check.Dockerfile` and re-runs the script inside it after the same provisioning CI does, dispatching before the EXIT trap so a bare invocation is unchanged by construction, guarding recursion via `PRE_TAG_CHECK_IN_CONTAINER`, and **failing loudly with no fallback** when no runtime is usable. Coordinator-verified: `--container` exit 0, git banner MATCH, 48 `test result: ok` / 0 FAILED; bare local run unchanged at exit 1 failing on the version witness alone; clippy clean. `src/`, `Cargo.toml` and `Cargo.lock` untouched, nothing `#[ignore]`d, no tag — v1.7.2 is a separate step. One NEW deliberately-open flake reported not fixed: a BrokenPipe at `tests/envelope_interior_path.rs:1068`, a different site from the known `envelope_tracer.rs` `run_stub` race, seen once under full load and 0/30 uncontended. Previously: 2026-09-17 - Completed quick task 260917-nhc: the LAST flake on the v1.7.1 publish gate — `driver::run::tests::the_current_group_agrees_with_the_proc_parse` — is closed, and the **recorded diagnosis was wrong again**, in both of its load-bearing clauses. `260908-uqq/deferred-items.md` wrote that "No test in the tree calls `establish_own_group()`, so the cross-test `setpgid` hazard the test's own doc names is not the mechanism" and that "`getpgrp()` cannot return a stale value, so the suspect is the `/proc/<pid>/stat` field parse under the load of many test binaries running concurrently". The `/proc` parse is EXONERATED and the cross-test `setpgid` hazard IS the mechanism. No test calls `establish_own_group()` DIRECTLY — but `src/driver/mod.rs:1834`, the "visible twin, driven end to end" arm of `a_target_phase_that_renders_as_another_is_refused_at_the_seam`, reaches it TRANSITIVELY through `drive` -> `execute_run` (`src/driver/run.rs:2553`), once per `LOOK_ALIKE_PAIRS` entry, and on Linux `setpgid(0, 0)` resolves against the THREAD-GROUP LEADER rather than the calling thread, so a libtest worker thread moves the WHOLE shared binary's group while the flaky test straddles it with two separate reads. **A TEST-SIDE race — production `/proc` parsing and group tracking are correct, and `src/` outside the `run.rs` test module is untouched.** `strace -f -e trace=setpgid,execve` caught 7 `setpgid(0, 0) = 0` calls from one TID that never `execve`s while the main pid's stream carried none; a non-job-leader parent polling `/proc/<child>/stat` watched the group transition from the inherited parent group to the child's own pid mid-run — which is also why it never reproduced interactively, since shell job control already makes a directly-launched binary its own group leader and the `setpgid` is then a no-op. Fail-first MEASURED before any edit: 0 failures in 39 direct runs vs 3 in 80 contended (5 in 140 overall, ~3.6%), every failure carrying the predicted constant-left/per-run-right signature. Fixed with a seqlock read in a documented helper — `current_group()`, the `/proc` parse, `current_group()` again, returned only when the bracketing reads agree, 25ms polls inside the loop, expiry PANICS by name rather than fabricating a pair. Nothing weakened: the `assert_eq!` and its message are BYTE-IDENTICAL, the `.expect` is retained and explicitly not a retry condition, no `#[ignore]`, no `--test-threads=1`, no bare `sleep`, no equality widened. After: 0 failures in 200 contended runs plus an independent 0 in 80 coordinator re-check, with the retry branch provably EXERCISED (7 firings in 7 runs, absorbed on attempt 1 each time). Gates: ten `rtk proxy cargo test --no-fail-fast` runs at 2120 passed / 1 failed / 15 ignored each, the target test green in all ten and no BrokenPipe in any; the one failure is the environmental git-version-constants pin. `./scripts/pre-tag-check.sh` exits 1 with gate 1 SKIPPED, gates 2/3/5 PASS and gate 4 failing on exactly one test — the version witness alone. Clippy clean. Module header gained a cross-test `setpgid` hazard section; phase 21's `deferred-items.md` closed the row append-only (277 insertions, 0 deletions) quoting both superseded sentences verbatim. Two non-goals flagged for audit: `execute_run`'s `establish_own_group()` ordering was deliberately NOT changed (T-nhc-05, `accept`), and the mover test was not stopped. `Cargo.toml`, `.github/` and tags untouched — v1.7.1 remains a separate step. Ran unisolated on the primary checkout after the #1941/#48 worktree base-check auto-degrade. Previously: 2026-09-17 - Completed quick task 260917-lkg: the last open flake on the v1.7.1 publish gate — `tests/envelope_tracer.rs::a_relocated_copy_of_the_stub_refuses_instead_of_acting`'s `Text file busy` (ETXTBSY) failure — is closed, and the **recorded diagnosis was wrong in an instructive way**. `deferred-items.md` called it "the classic write-then-exec race", which points at the `std::fs::copy` immediately before the second exec. `strace -f -e trace=execve` was run against a live reproducer and caught the failing syscall three times; **every one of them was the SANCTIONED stub** in the envelope's own hooks directory, and the relocated copy's path never appeared in a failing `execve` at all. What flaked is the CONTROL leg, and the copy is a bystander. The writer is a `NamedTempFile` descriptor from `src/envelope/hooks.rs::write_stub` that a SIBLING libtest thread's forked child inherits: `O_CLOEXEC` closes it at that child's own `execve` but not one instruction earlier, and in that window the child is a writer on the stub's inode. ETXTBSY is a per-INODE condition (`i_writecount`), which is what rules out the tidy-looking repair — closing or syncing the `File` before `persist`'s rename changes nothing, because `rename(2)` does not change the inode and the inode was already exposed during the pre-rename write and chmod. Two other eliminations were considered and rejected in writing (a `hard_link` for the copy, which the strace exonerates and which would share the sanctioned inode; a process-wide RwLock serialising writes against spawns, which would need every `Command::spawn` in the binary wrapped and would silently reinstate the flake from any missed site while *looking* deterministic). The descriptor is therefore not ours to close, so `run_stub` waits it out: a bounded, **ETXTBSY-only** retry on `spawn()` — `Instant::now() + Duration::from_secs(30)` with 25ms polls, the `*_within` idiom `tests/driver_reattach.rs` established in `260917-k6y` — where every other spawn error still fails on the spot under the verbatim `the generated stub is executable` wording, and where **expiry PANICS rather than skipping**, because an exec that never happened proves nothing about refusal and `Text file busy` may never stand in for the refusal being asserted. **Nothing was weakened**: 33 asserts after against 29 before, zero removed, no `#[ignore]`, no `--test-threads`, no fixed pre-assertion `sleep`, no `src/` change. Fail-first was MEASURED before a byte was edited: 0 failures in 70 isolated runs and 0 in 6 full-suite runs, then **10 failures in 200 runs (5%)** under an 8-way contended reproducer, every one this test, this line, this error string. After the fix: **0 ETXTBSY failures in 1000 contended runs** (the executor's 4×200 plus an independent 200-run coordinator re-check), with the retry branch provably EXERCISED — 8 absorbed retries logged across those runs, so this is proven in effect rather than merely absent. Gates: nine `rtk proxy cargo test --no-fail-fast` runs → 2120 passed / 1 failed / 15 ignored in eight of them and 2119/2/15 in one, the target test green in ALL nine and `Text file busy` in none; the constant failure is the environmental git-version-constants pin (installed git 2.53.0 against constants derived against 2.55.0, expected green on the runner) and run 7's extra is the separate, per-run-flaky `driver::run::tests::the_current_group_agrees_with_the_proc_parse`, reported rather than absorbed. `./scripts/pre-tag-check.sh` exits 1 with gate 1 SKIPPED, gates 2/3/5 PASS and gate 4 failing on **exactly one** test — the version witness alone, which is the state a correctly-prepared v1.7.1 is supposed to be in. `rtk proxy cargo clippy -- -D warnings` clean. The paper record moved with the code: the module header now carries an ETXTBSY section explaining why the copy was exonerated and what the wrong fix would have been, and phase 21's `deferred-items.md` has its `envelope_tracer` half closed append-only (253 insertions, **0 deletions**), quoting verbatim the sentences it supersedes — including `260917-k6y`'s own "stays OPEN" — so the two-binary item is now fully closed. **One NEW, distinct race was found and deliberately left OPEN**: `the stub reads its stdin: BrokenPipe` at `run_stub`'s `write_all`, seen 1 in 800 contended runs and 0 in nine full-suite runs, not introduced here (that block is byte-identical before and after) and not fixed because the plausible repair touches an assertion, which was a hard prohibition; it is registered with its measurements in `deferred-items.md` per the T-19-91 precedent. `Cargo.toml`, `.github/`, `src/` and tags all untouched — v1.7.1 remains a separate step. Ran unisolated on the primary checkout after the #1941/#48 worktree base-check auto-degrade. Previously: completed quick task 260917-k6y: `tests/driver_reattach.rs`'s two long-standing flakes are closed, and the fix is a synchronisation fix rather than a tolerance. The mechanism is candidate **M2** from the file's own header, now confirmed: `live_within` polls `driver::liveness::is_run_alive`, which is a read of `/proc/<pid>/cmdline`, and the kernel populates that file at `execve` — microseconds after `spawn()`, long before the driver has established its envelope, taken its lock, written `run.json` or emitted one journal record. The tests were therefore waiting on the PROCESS and then immediately reading an ARTIFACT the writer had not produced, which is precisely why a failing run took ~0.5s against ~6.1s for a passing one: the assertion won the race. Three bounded artifact waits now sit beside `live_within`/`gone_within` in the file's own `*_within` idiom — `observed_within` (polls `reconcile::reconcile_all` until the run **id** appears, matching the id rather than `len() == 1`), `journal_record_within` (requires a line that actually parses as `ParsedLine::Record`, because `reader::tail_lines` returns `Ok` with zero lines for a file that does not exist, so an `is_ok()` test would have returned true instantly and fixed nothing) and `run_json_within` (existence ONLY, so a completed run cannot satisfy the wait that the still-null `ended_at` assertion depends on). Each reuses `live_within`'s deadline + 25ms shape at `Duration::from_secs(30)` — generous for a contended 2-core `ubuntu-latest` runner and costly only on a genuine failure — each is layered AFTER the existing `live_within` assertion rather than replacing it, and each fails naming exactly what never appeared. **Nothing was weakened**: no assertion deleted or loosened, no `#[ignore]`, no `--test-threads=1` workaround, no `sleep`, no `src/` change, no absence treated as success. Fail-first was reproduced at HEAD before a byte was edited (1 red of 6 isolated runs, at 0.53s, with both arms firing together), and 10/10 isolated runs are green afterwards at ~6.24s with none at the ~0.5s failure signature. Gates: five `rtk proxy cargo test --no-fail-fast` runs → 2120 passed / 1 failed / 15 ignored in four of them and 2119/2/15 in one, both `driver_reattach` tests green in ALL five; the constant failure is the environmental git-version-constants pin (installed git 2.53.0 against constants derived against 2.55.0, expected green on the runner) and run 4's extra is the separate, still-OPEN `tests/envelope_tracer.rs` `Text file busy` write-then-exec flake, reported rather than absorbed. `./scripts/pre-tag-check.sh` exits 1 with gate 1 SKIPPED, gates 2/3/5 PASS and gate 4 failing on **exactly one** test, down from three — the reduction this task was for. `rtk proxy cargo clippy -- -D warnings` clean. The paper record moved with the code: the module header was rewritten from "THIS FILE IS FLAKY, AND `--test-threads=1` WILL NOT HELP YOU" into a closed case that records why process liveness was the wrong synchronisation point, while **M1 and M3 are left explicitly open, unmeasured and unclaimed** and the `--test-threads=1` correction plus the historical data points are preserved as the record of how the wrong answer was reached twice; phase 21's `deferred-items.md` has its `driver_reattach` half closed append-only (221 insertions, 0 deletions) with `envelope_tracer` left OPEN; and `.planning/todos/pending/2026-08-18-driver-reattach-spawn-artifact-race.md` was retired to `completed/` by pure `git mv`. `Cargo.toml`, `.github/`, `src/` and tags all untouched — v1.7.1 remains a separate step. Ran unisolated on the primary checkout after the #1941/#48 worktree base-check auto-degrade. Previously: completed quick task 260917-jdi: `scripts/pre-tag-check.sh` now reproduces `.github/workflows/release.yml`'s gates locally, before a tag exists, and CLAUDE.md's Release Process step 3 runs it instead of the bare `cargo build && cargo test && cargo clippy` chain. This is the prevention chosen for the v1.7.0 publish-gate failure and deliberately the MINIMAL one — **no CI workflow was changed**. Gates, in order: tag/version match (`cargo metadata --no-deps --format-version 1 | jq -r '.packages[0].version'` against the argument with the same `${ARG#v}` strip CI uses; no argument reports the crate version rather than failing), the msrv job's `cargo +<floor> check --all-targets --locked`, `cargo build --release`, `cargo test --no-fail-fast`, `cargo clippy -- -D warnings`. Three things carry the design. Gate 4 uses `--no-fail-fast` where CI uses plain `cargo test`, on purpose and commented as such: CI's fail-fast stopped at the lib binary and the later suites never ran, which is exactly how the v1.7.0 breakage stayed invisible. The MSRV floor comes from `cargo metadata`'s `rust_version` and the toolchain is detected READ-ONLY via `rustup toolchain list` — never probed with `cargo +<floor> --version`, which auto-installs the missing toolchain and so both mutates the machine and lies about the skip condition; an absent floor prints a marked `SKIPPED`, never a silent pass. And the git-version reconciliation block greps `CONFIG_SECTION_CONSTANTS_DERIVED_AGAINST_GIT_VERSION` out of `src/envelope/policy.rs` at runtime, so no version literal is baked into the script and it cannot go stale across the next re-derivation; an unparseable constant is a hard error, because the script has then lost its subject. **A git/constant mismatch is a loud ADVISORY, never a script failure** — banner before the gates and repeated in the EXIT-trap summary so it survives a long scrollback — because local git 2.53.0 against a constant of 2.55.0 IS the current, correct state of a properly-prepared release, and hard-failing there would train the operator to ignore the one warning that matters. The banner says the three facts plainly: this run cannot validate that test the way CI will, the GitHub `ubuntu-latest` runner's ambient git is the AUTHORITY for it, and a green local gate does not guarantee the publish job passes. The version witness's consequent local failure is EXPLAINED in the summary rather than filtered — a filter there is the same blindness the task exists to remove. On this tree the script exits 1: gates 1/2/3/5 PASS, gate 4 FAILED on exactly the three known pre-existing failures (the version witness plus the two documented `tests/driver_reattach.rs` flakes), zero gates `NOT REACHED`. `Cargo.toml`, `.github/`, `src/` and tags all untouched — v1.7.1 remains a separate step. Ran unisolated on the primary checkout after the #1941/#48 worktree base-check auto-degrade. Previously: completed quick task 260917-ii4: the git config-section constants are re-derived against `git version 2.55.0`, the version the release-CI runner carries — the re-derivation the version-witness pin's own failure message prescribes, performed rather than skipped, which is what unblocks the v1.7.0 release CI. All twelve `Documentation/RelNotes/2.44.0.adoc` … `2.55.0.adoc` were read and cross-checked against master's `config.adoc`, `config/alias.adoc`, `config/hook.adoc`, `config.c` and `alias.c`. **NEITHER `INDIRECTION_SECTIONS` NOR `REPARSED_COMMAND_SECTIONS` CHANGED, and that is the finding**: no third configuration-splicing section appeared (`git_config_include()` still splices on exactly `include.path` and `includeif.<cond>.path`) and no second section whose value git re-parses as a git command line appeared (`alias.c` still has one `parse_config_key(var, "alias", …)` call site). The load-bearing find is that git 2.54 gave `alias` a SECOND SPELLING — the three-level `alias.<name>.command`, with arbitrary-UTF-8 names in a raw-byte case-sensitive subsection — which adds no member because its SECTION is still `alias`, so the existing section-only rule refuses it with no code change, while a guard that had enumerated the two-level `alias.<name>` shape would have failed OPEN on it. That also falsified the doc claim that "`alias` has none" [no subsection], repaired in place. Exactly one executable line changed in the task (the version literal); every other changed line begins with `///`, and the four `Measured against \`git version 2.43.0\`` probe tables are byte-identical, because the 2.44→2.55 re-derivation was DOCUMENTARY and rewriting them would fabricate a measurement never taken. Gates: `cargo build` pass, `cargo clippy -- -D warnings` pass, `rtk proxy cargo test --no-fail-fast` 48 suites / 2119 passed / 2 failed / 15 ignored with the failing set identical before and after. **The version pin now fails LOCALLY BY DESIGN** (derived against 2.55.0 vs the installed 2.53.0) and is expected green on the runner's 2.55.0 — it is a schedule aimed at CI, not a regression; the second failing slot is a per-run-flaky process test, measured across three re-runs each way. `Cargo.toml` untouched, no tag, no release — v1.7.1 is a separate step. Ran unisolated on the primary checkout after the #1941/#48 worktree base-check auto-degrade. Previously: completed quick task 260917-hc3: the `drive` CLI subcommand now carries clap's `#[command(hide = true)]`, so it is absent from `--help` (verified against the real binary: `gsd-meta-manager --help` lists only `add`/`remove`/`list`/`help`) while the parser is untouched — `gsd-meta-manager drive <alias> --command /gsd-progress` still reaches the handler and fails on the alias lookup, not on clap. This is help-visibility only: `drive` is deliberately still NOT gated on `GSDMM_EXPERIMENTAL_FEATURES`, because the TUI respawns itself as `current_exe() drive` into `/dev/null` stdio and a refusal would break that path invisibly; `src/driver/spawn.rs` and the envelope env scrub were not touched. Two tests in a new `mod tests` in `src/cli.rs` pin both halves (`the_drive_subcommand_is_absent_from_rendered_help`, with a positive control on the still-visible `list` entry, and `the_drive_subcommand_still_parses_and_resolves`). Gates: `cargo build` pass, `cargo clippy -- -D warnings` pass, `cargo test --no-fail-fast` 2120 passed / 1 failed (+2 over the 2118/1 baseline, failing set identical) — the known-environmental `envelope::policy` git-version-constants test (installed git 2.53 vs constants derived against 2.43). Ran unisolated on the primary checkout after the #1941/#48 worktree base-check auto-degrade. Previously: completed quick task 260917-fko: the driver/autonomous surface is now behind a `GSDMM_EXPERIMENTAL_FEATURES` startup flag, default OFF, hidden entirely rather than visible-but-disabled — 28 gate points threaded off one `AppContext.experimental` field read once in `App::from_config`; with the flag on the Driver pane title and the help Driver heading carry the literal `EXPERIMENTAL`. `driver_opt_in` and `driver_max_concurrent` are untouched outer-layer-wise and pinned by test; the `drive` CLI subcommand is deliberately NOT gated (the TUI respawns itself as `current_exe() drive` and env propagation through the envelope scrub is not guaranteed) — recorded as an inferred decision for audit. Gates: `cargo build` pass, `cargo clippy -- -D warnings` pass, `cargo test --no-fail-fast` 2118 passed / 1 failed (+31 tests over the 2087/1 baseline, no new failure) — the one failure being the known-environmental `envelope::policy` git-version-constants test (installed git 2.53 vs constants derived against 2.43). Ran unisolated on the primary checkout after the #1941/#48 worktree base-check auto-degrade. Previously: completed quick batch 260916-vqv (6 of 6 items). This resume run shipped the last two: 260916-vqy (derive a phase's execution waves from PLAN.md frontmatter and group the Pipeline tab by them; multi-wave marker in the phase list) and 260916-vqz (`b` on the dashboard opens the selected project's detail view already on a populated Backlog tab, documented on its own help row). Gates on the final tree: `cargo build` pass, `cargo clippy -- -D warnings` pass, `cargo test --no-fail-fast` 48 suites / 2087 passed / 1 failed — the known-environmental `envelope::policy` git-version-constants test (installed git 2.53 vs constants derived against 2.43). Both items' SUMMARY.md carry a "Coordinator inferred decisions (for audit)" section: the batch resumed past a self-inflicted base-revision divergence, the two items were serialized (not parallel) because both touch `src/ui/screens/detail.rs`, and 260916-vqz ran unisolated on the primary checkout after the #1941/#48 worktree base-check auto-degrade.
