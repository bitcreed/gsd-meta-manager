...
## Plan 19-27 — the rules and the corrections (round 10's second half)

**`/gsd-secure-phase 19` is NOT cleared by this plan.** `T-19-86`, `T-19-91` and
`T-19-111` all remain OPEN at `high`. Only the WRAPPER-OPERAND sub-class of
`T-19-60` is closed.

### `T-19-112` — the SAFE-06 ledger as a FILE — **NARROWED, NOT CLOSED**, `high`

**The clause that narrowed it:** `policy::envelope_carrier_operand`, raised once at
the top of `hooks::classify_segments`' existing per-segment loop **before** the
resolution match, refusing a segment any of whose words is ABSOLUTE, LITERAL and —
after LEXICAL normalisation with `..` collapsed textually and no link followed —
equal to or under `envelope_dir_in(root, alias)` compared COMPONENT-WISE. Reason
`ParkReason::EnvelopeAssertionFailed`, no new variant. So `rm -f <ledger>`,
`truncate -s 0 <ledger>`, `cp /dev/null <ledger>`, `shred -u <ledger>` and
`rm -rf <env>/<alias>` are refused, and so are `cat <ledger>`, `wc -l <ledger>` and
`ls <env>/<alias>/hooks` — the disclosed over-refusal, a decision with a stated
reason.

**Narrowed in ONE ROUTE OF SEVERAL, and the others are named:**

- **the four fail-open directions**, none with an automated control: (i) a
  REDIRECTION TARGET is not an operand — real by mechanism, `tokenize` deletes the
  operator AND its target; (ii) an EXPANSION-BORNE operand cannot be resolved;
  (iii) a SYMLINK is not followed — **NARROWED** by `ln -s <env>/…` naming an
  envelope path as its own operand; (iv) a RELATIVE path is not resolved —
  **NARROWED** by `cd <env>/<alias>`, with the composite refused BY SEGMENT ONE
  and segment two still unresolvable. **No pin, no schedule and no version witness
  was created for any of the four**, because they are reachable by the agent TODAY
  and a schedule would observe the wrong thing.
- **`C-15`** — `~/.config/gsd-meta-manager/config.json`'s unclamped `pr_cap_*`,
  re-read on every Bash tool call. It disarms SAFE-06 **by raising a number, with
  no ledger write and no envelope path named**, so rule (a) cannot reach it. A
  DISTINCT ROUTE, control (e), no rule, no acceptance. **`pr_cap_*` is
  deliberately NOT clamped** — a product decision this security round declines to
  make silently.
- **the DEFERRED option (b)**, below, which leaves the guard as the cap's only
  observation point (`T-19-35`, `AR-19-05`).

### `T-19-113` — the hook stubs as FILES — **NARROWED, NOT CLOSED**, `medium`

Covered by the same clause, because the boundary is the envelope DIRECTORY rather
than a filename list: `cp /bin/true <hooks>/pre-push`, the `pre-commit` twin and
`rm -rf <hooks>` are all refused. **The same four directions remain open**, and
`printf 'exit 0' > <hooks>/pre-push` — direction (i) — is pinned PERMITTED before
and after.

**Option (b) is recorded STRUCTURALLY UNAVAILABLE here**: the thing that would
detect a replaced `pre-push` is the binary the replacement removed from the path.
`T-19-01`'s `current_exe()` + `assert_provenance_in` mitigation certifies a
RELOCATED COPY, is correct, and is **not weakened** — it is simply not a defence of
the sanctioned path's CONTENTS.

**D-09's narrative is corrected regardless**, in `advisory.rs:254` and
`cred.rs:241-244`: the ceiling now names the FILE route as well as the
`GIT_CONFIG_COUNT` route. `mod.rs:165-168` needs the same correction and was **NOT
opened** — it is outside this plan's `files_modified`. **Recorded as a
documentation gap for a later round.**

### Option (b) for the ledger — **COSTED AND DEFERRED, with NO schedule attached**

The ledger is append-only by design and its entries carry `run_id` and `at`, so a
deletion or truncation IS detectable at the next append — the material exists.
**What it costs**: a SECOND CARRIER for SAFE-06, which `hooks.rs:1421-1428`,
`T-19-35` and `AR-19-05` all say in terms SAFE-06 does not have; and a stated
latency rule, *"the ledger read is a single pass over one append-only file"*,
because the cross-check would read the run journal too. **A round of its own. No
pin, no revisit condition and no witness is created for it, and (a)'s residue
stays uncovered.**

### `T-19-114` — the corpus's carrier alphabet — **CLOSED for the control-carrier axis**, `low`

`19-26`'s fifth named axis (`CONTROL_CARRIER_CLASSES`) drew a carrier that is a
FILE, off the argv plane nine rounds had modelled. Its fail-closed property is
green after this plan and its six verdict-preserving alphabets are still
PERMITTED. **Successor: the axis this corpus does not draw is a carrier reached
through a spelling the guard cannot see at all** — a redirection target, an
expansion-borne word, a link made before the run — and no corpus can turn those
red without a rule that could pass them, which is why they are recorded as
residue rather than as an axis. **Ten consecutive rounds have found this failure
mode**; the eleventh should assume it exists rather than hope it does not.

### `T-19-111` — a `.git/config` written by any means — **OPEN at `high`, NO RULE**, attribution corrected at all five sites
...
`REPARSED_COMMAND_SECTIONS`' own K1 class reached through a carrier outside argv.
Measured and live: a `printf`-written `.git/config` alias carrying
`-c include.path=<f> push --force` rewrote a bare remote's `main`, with a
no-carrier control beside it that left it unmoved.

Five sites: `policy.rs:1177-1183` **edited**; `cred.rs:287-289` **edited**, with
the *"or one written by a means that is not `git config`"* half **RESTORED**;
`19-SECURITY.md:7430` and `19-25-SUMMARY.md:265` **corrected BESIDE** in this
plan's appended `19-SECURITY.md` subsection, never by editing them; and the
`T-19-108` entry below.

**`T-19-111` is moved OUT of `T-19-86` and never into it. No rule is written for
it, and no acceptance is made for either — that is a human decision.**

### `T-19-108` — the closure entry corrected (site 5 of the attribution correction)

The plan-19-25 entry above records `T-19-108` as **CLOSED AS SCOPED** and its
"as scoped" names the excluded **BODY** — the `!` arm's carve-out. **It does not
name the excluded CARRIER, and it should.** The closure covers the `-c`, the
`--config-env` and the `git config` WRITE-OPERAND deliveries of a non-`!` alias
body, all three of which the guard sees on the argv it reads. **It does NOT cover
the same K1 body arriving through a FILE the guard never saw a write to** — a
`.git/config` written by `printf`, `sed`, `cp` or a redirection, or one predating
the run. That carrier is `T-19-111`, it is open at `high`, and this plan writes no
rule for it. The correction is made here, beside the entry, rather than by editing
it.

### `SECTION_ENVELOPE`'s `Guaranteed` cap clause — **REPAIRED**

From `Pull-request cap: an append-only ledger this repository does not contain.`
to `The pull-request count lives in / an append-only ledger this repository does
not contain.` — a LOCATION FACT that is mechanically true rather than a claim
about a control that measurement falsified. **It makes no completeness claim and
enumerates nothing**, because `C-15` is a fifth route and the deferred option (b)
a sixth. 213 tokens against the unraised 215 cap; the three pinned phrases in
order and unwrapped; `\x20` indentation; no residual disclosure dropped. **Exactly
one assertion updated**, the one `19-26` named in advance;
`tests/envelope_advisory.rs` shows a ZERO diff and all ten of its tests pass
unmodified.

### The proportional-floor comment — **corrected**

Cited *228,785 bytes, 78.7%*; measured **262,229 bytes at base** and **277,570
after this plan's additions**, ratio **64.8%**. Audit 9's 261,387 and the mandate's
261,386 are **CHARACTER** counts — `production.len()` is `String::len()`, which is
bytes, and the file carries 842 multi-byte characters. **The FLOOR stays at
`180_000` and its strictness is unchanged.**

### Carried forward UNFIXED, and stated rather than left as an omission

- **`C-08` — `<env>/<alias>/settings.json`'s claimed-but-unwired second carrier.**
  `19-26` measured and registered it; **repairing a delivery is a change to the
  SPAWN SEAM and is out of this round's scope.** Its behavioural half remains
  **UNMEASURED** and is claimed in neither direction.
- **`C-05`, `C-07`, `C-12`, `C-15`** — the planner-derived carriers, carried
  forward at their measured severity with control (e). No rule, no acceptance.
- **`AR-19-04`'s reasoning gap** — *"regenerates it at each run start"* does not
  cover a write DURING the run. **RECORDED beside it and nothing more; NOT
  un-accepted.** `AR-19-05` likewise. No `AR-` row was added, edited, renumbered,
  accepted or un-accepted.

### Still OPEN and unchanged

- **`T-19-86`** — OPEN at `high` by explicit user scoping decision. Its four
  registered rows and its persisted-alias arm still exit 0, its pins green and
  UNMODIFIED. **`T-19-111` is moved OUT of it, not into it.** Not fixed, not
  narrowed, not re-scoped.
- **`T-19-91`** — OPEN at `high`, arms unweakened, no decision-operand rule added
  and no denylist extended.
- **`T-19-96`**, **`T-19-110`** — open at `medium`, left exactly as pinned.
- **`T-19-74`** — core rows frozen; `env $X push --force origin main` and
  `X=git; env $X push --force origin main` still at exit 0.
- **`T-19-84`, `T-19-85`, `T-19-61` … `T-19-73`** — open and unaccepted by explicit
  user decision. Untouched apart from the three named file exceptions
  (`hooks.rs`'s single call site, `cred.rs`'s doc-only paragraph, `advisory.rs`'s
  single constant).
- **the `glab --host` forge cell** — carried forward UNFIXED, **`--hostname` KEPT**
  in `FORGE_VALUE_OPTS`. `glab` is confirmed NOT installed, so a pin over that cell
  cannot run against its real callee and a pin that skips is fail-open.
- **the `T-19-17r` bookkeeping gap** — still **OUTSTANDING**. `grep -cE '^\| AR-19-13 \|'`
  is **0**, verified after writing. **This plan adds no Accepted-Risks-Log row,
  creates no `AR-19-13`, and does not apply the word "accepted" to it.** Eight
  agents have deliberately left the acceptance unmade; **this plan is the ninth.**
- **`src/envelope/mod.rs:165-168`** — D-09's ceiling paragraph, needing the same
  FILE-route correction `cred.rs` and `advisory.rs` received. **NOT opened; a
  documentation gap for a later round.**
- **the three documented flakes** — the two `tests/driver_reattach.rs` failures and
  the `tests/envelope_tracer.rs` ETXTBSY stub-write race. **One FIRED during this
  plan's verification** —
  `a_run_killed_without_an_ending_is_reported_crashed_and_nothing_on_disk_is_repaired`
  — and was neither fixed nor worked around. It did not fire on the final gate run;
  absence is not evidence it is fixed.

## Plan 19-28 — round 11's CORPUS (the rules are `19-29`'s)

**This plan closes NOTHING.** It measured four cells audit 10 left on round 10's
plane, widened the corpus so the axis can fail on each, observed it RED and
stopped. `T-19-115` … `T-19-118` are ALL OPEN at this plan's end, and whether
`19-29` closes any of them is audit 11's judgement. **`/gsd-secure-phase 19` is
NOT cleared by this plan, by `19-29`, or by the two together** — `T-19-86`,
`T-19-91`, `T-19-111` and `T-19-112` all remain open at `high`.

### `T-19-116` — `C-10`, the binary every stub and the guard itself exec — **high, OPEN**

**`C-10` GETS ITS FIRST DEFERRED-ITEMS ROW HERE, AND THE ABSENCE OF THAT ROW IS
THE FINDING.** `19-26` enumerated it and marked it UNOBSERVED; `19-27`'s
disposition table has **no row for it at all** — no threat id, no severity, no
entry here and no control letter. That is how the widest of the fifteen carriers
survived a round that enumerated it.

| Field | Value |
|---|---|
| Carrier | `C-10` — the binary baked into `stub_body` (`hooks.rs:196-204`) AND into `guard_command` (`hooks.rs:1518-1524`) |
| Severity | **high** |
| Control | **(a), EXACT PATH, PARTIAL** — `19-29`'s widened path set |
| Status | **OPEN.** Corpus RED at `19-28`'s end |

**Driven end to end against real git in a REBUILT bare-remote fixture, with a
control on both sides and layer 2 measured separately:**

```text
CONTROL  real binary      git push --force  -> REFUSED, remote main 38b1ae0 UNMOVED
LEG B    cp /bin/true <binary>, same push   -> COMPLETED, main 38b1ae0 -> 153e6c1 MOVED
CONTROL  binary restored, same push         -> REFUSED, remote main 38b1ae0 UNMOVED
LAYER 2  <replaced binary> envelope guard alpha  on a force push -> exit 0 (PERMIT)
         <real binary>     envelope guard alpha  on the same     -> exit 2
```

**Layer 2 AND layer 3 in one permitted call.** Strictly wider than `C-02`
(`T-19-113`, `medium`, layer 3 only) and wider than `env -u GIT_CONFIG_COUNT`
(layer 3 only, **and REFUSED** at `hook_bypass_blocked`).

**`T-19-01`'s mitigation is correct for what it covers and covers nothing here**:
...
**The boundary is an EXACT PATH and not a directory prefix**, because the
binary's directory is shared with everything else the user installed. The
near-miss controls `cp /bin/true <BINARY-PARENT>/some-other-file` and `ls
<BINARY-PARENT>` are measured exit 0 and asserted exit 0 AFTER; a prefix clause
turns both red and would refuse `ls ~/.cargo/bin` and every `cargo install`.

**Residue that stays OPEN after `19-29`, disclosed rather than closed:** the
`$(command -v …)` spelling (direction (ii)), a tilde spelling (direction (v)), a
relative spelling after a `cd` — and **a write naming a SYMLINK to the binary**,
which is direction (iii) over the binary. `std::env::current_exe()` reads
`/proc/self/exe`, which is fully resolved, and `hooks::install` bakes the same
call's answer, **so the baked path and the guard's path agree by construction**;
the link path is a third string neither covers.

### `T-19-118` — `C-05`'s credential half, via fail-open direction (i) — **high, OPEN**

**`T-19-118` IS `T-19-23` REOPENED.** `T-19-23` is `high` and `closed`, and its
declared mitigation is precisely the control that consists of the generated file
naming no helper.

`printf '[credential]\n\thelper = store\n' >> <ENV>/<alias>/gitconfig` is **exit
0** through the guard with an EMPTY walk — a REDIRECTION, which rule (a) cannot
see — and `git credential fill` for `https://github.com` then returns the ambient
username and password from `~/.git-credentials`. **Against a CONTROL in which the
same file, exactly as `write_gitconfig_in` leaves it (`cred.rs:393-415`), answers
`git config --get credential.helper` at exit 1 with no helper and `git credential
fill` at exit 128 with the secret ABSENT.** The secret is recorded PRESENT/ABSENT
and never transcribed (SAFE-04's own reasoning).

**Both halves of `SECTION_ENVELOPE`'s FIRST `Guaranteed` clause are false**: the
generated file DOES name a credential helper, and the run DID reach the ambient
credentials. **`19-27` repaired the `Guaranteed` CAP clause and left this
`Guaranteed` CREDENTIAL clause unqualified, one clause over from the repair,
although `19-26` had already recorded the falsification.**

Six write spellings, four reached and two not:

```text
exit=2  cp / sed -i / tee -a / shred -u   <ENV>/alpha/gitconfig    OPERAND
exit=0  printf … >> <ENV>/alpha/gitconfig                          REDIRECTION
exit=0  cat /tmp/evil >> <ENV>/alpha/gitconfig                     REDIRECTION
```

**`AR-19-04`'s reasoning gap is RECORDED and the acceptance is NOT un-accepted.**
It accepts `T-19-29` (*"agent rewrites the generated git config"*, `medium`) on
the rationale *"the envelope regenerates it at each run start"*, which does not
cover a write DURING the run. **The acceptance is about TAMPERING at `medium` and
says nothing about credential REACHABILITY at `high`.** `T-19-118` is registered
as its OWN row rather than moved into `T-19-23` or into `AR-19-04`. **No `AR-`
row is added, edited, renumbered or un-accepted; un-accepting a risk is a human
decision.**

**The candidate control REPRODUCES and `19-29` REQUIRES it.** Injecting a second
`GIT_CONFIG_COUNT` pair naming `credential.helper` with an EMPTY value makes
`git credential fill` fail closed. **The criterion is `git credential fill` and
NOT `git config --get-all credential.helper`**, which still prints `store` (plus
an empty line, exit 0) — git's empty value resets the helper list that RUNS, not
the list the config query ENUMERATES, so a `--get-all` metric reports a working
control as broken. Cost measured: `GIT_ASKPASS` UNTOUCHED, `gh` unaffected, and a
later `-c credential.helper=store` overrides the reset — **a bounded residue,
bounded because it is ARGV-VISIBLE and already governed.** It reads no command
line at all, so it defends `C-05`'s credential half against all seven spellings
rather than one.

### `T-19-115` — rule (a) fails open in SEVEN directions, not four — **medium, OPEN**

**The docs say FOUR and every cited spelling is `$`-shaped** (`policy.rs:5293-5325`,
`hooks.rs:1013-1017`). **There are SEVEN. Replacing the four-direction statement
with the seven, as this plan's own section states it:**

```text
(i)   a REDIRECTION TARGET       operator and target deleted before execve
(ii)  an EXPANSION-BORNE operand the value comes from a substitution
(iii) a SYMLINK                  resolved lexically, links not followed    NARROWED
(iv)  a RELATIVE path            the guard has no cwd                      NARROWED
(v)   a TILDE `~/…`              not `Token.literal`                       NEW
(vi)  a GLOB `* ? [`             not `Token.literal`                       NEW
(vii) a BRACE LIST `{a,b}`       not `Token.literal`                       NEW
```

**The three unnamed ones need no prior read, no symlink and no cwd.** Measured
live, each beside its absolute-literal twin at exit 2:
`rm -rf ~/.local/share/gsd-meta-manager/envelope/alpha` exit 0;
`rm -rf <ENV>/alph?` exit 0, nine carriers in one call; `rm -f <ENV>/alpha/*`,
`unlink <ENV>/alpha/pr-ledger.ndjso?` and
`rm -f <ENV>/alpha/{pr-ledger.ndjson,x}` all exit 0. **All three were also drawn
in REDIRECTION-TARGET position — the cell nobody had drawn.**

**`bash` confirmed the shell reaches the file for every OPERAND spelling.** In
redirection-target position the tilde reaches and the glob reaches **only when it
matches exactly one path** (two matches → `ambiguous redirect`), while
**`: > <ENV>/alpha/{pr-ledger.ndjson,x}` does NOT reach** — brace expansion
produces two words and a redirection target must be one. That row stays in the
corpus as verdict-preserving and the record says it is a permit that costs
nothing rather than a bypass. `sh` (dash) does not glob a redirection target at
all; the guard is registered against the agent's `Bash` tool, so bash's semantics
are the relevant ones.

**`19-29` writes NO RULE for any of the three**, and the reason is mechanical: a
tilde needs the ENVIRONMENT and a glob needs the FILESYSTEM, and the guard is
forbidden both. **The fix is the residue's ARITHMETIC** — state the CONDITION,
do not enumerate spellings. **The docs still say FOUR at this plan's end;
`19-29` corrects them.**

**This is `T-19-76`'s failure mode for the TENTH consecutive round**, after
`T-19-83`, `T-19-89`, `T-19-95`, `T-19-99`, `T-19-101`, `T-19-105` and
`T-19-114` — **and for the first time it is in the cell the previous round just
filled.**

### `T-19-117` — the ledger's SIZE is unbounded work on the guard's critical path — **medium, OPEN**

`ledger::record_and_check_in` reads the ledger WHOLE (`ledger.rs:225-227`) and
`tally` (`:338-377`) walks every line, **with no size bound anywhere**, against
`GUARD_TIMEOUT_SECS = 5` (`hooks.rs:1408`, delivered at `:1504`).

```text
empty       0 B            gh pr create =   49 ms
10,000      1.01 MB        gh pr create =   96 ms
200,000     20.2 MB        gh pr create = 1008 ms
2,000,000   202 MB         gh pr create = **7900 ms**   PAST THE 5 s DEADLINE
                           ls 43 ms / git status 43 ms / git push --force 46 ms
```

**The cost falls ONLY on the forge commands the cap governs**, because only they
reach `record_and_check_in`. The inflation is delivered by direction (i) — `yes …
>> <ledger>`, `cat /tmp/big >> <ledger>`, `head -c 200000000 /dev/zero >>
<ledger>`, all exit 0.

**`ends_mid_line` (`ledger.rs:288-303`) refuses a whole-file read for a one-byte
question three functions above the unbounded read**, in its own words *"a
whole-file read to answer a one-byte question is the sort of thing that turns a
guard into a hang"*.
...
fail CLOSED**: a tail read, a line cap or sampling all UNDER-COUNT, which
violates `tally`'s own no-under-count invariant in the direction that permits.
The identifier is `envelope_assertion_failed`, not `PrCapExceeded` (D-24).

**THE BEHAVIOURAL HALF IS UNMEASURED** — what the agent CLI does with a
`PreToolUse` hook past its registered timeout is a property of a closed-source
binary — **and is claimed in NEITHER direction**, the same discipline `C-08`'s
behavioural half is held to.
...
`19-29`'s `files_modified`.** The test pins three rows PERMITTED —
`: > <ENV>/alpha/pr-ledger.ndjson`, `printf 'exit 0' > <ENV>/alpha/hooks/pre-push`
and `echo evil > <ENV>/alpha/askpass` — which are three of the four entries of
`CONTROL_CARRIER_REDIRECTION_TARGETS`, the alphabet `19-28` moved into the
...
which is the shape that halted `19-23` mid-plan.** `19-28` does not fix it —
editing that file is prohibited by this plan and round 10's evidence stays
attributable to the round that produced it. **`19-29` must either add the file to
its `files_modified` and move those three rows into an `after_19_29_…` property
with their reason stated, or report the scope conflict.** The other directions
pinned in the same file — (ii), (iii), (iv), and `: > /tmp/l` whose target is
outside the root — are verdict-preserving and unaffected.

### The `421` correction, stated BESIDE and not in place

**`deferred-items.md:2172`, inside the plan-19-27 section, says the file carries
*"842 multi-byte characters"*. That section is NOT edited.** Corrected here:
**842 is the byte-minus-character DIFFERENCE; the COUNT is 421**, each three-byte
character contributing two extra bytes. The same error is at
`src/envelope/policy.rs:7450`, **which is `19-29`'s to edit and which this plan
does not touch.** Every other number in that comment was re-derived by audit 10
and is right, and **the floor's value and strictness do not move.**

### `mod.rs:165-168` is put IN SCOPE for `19-29`

`ENVELOPE_ROOT_ENV`'s residual-limit paragraph argues the override *"does not
lower a boundary that was standing"* because the same party could unset
`GIT_CONFIG_COUNT`. **`T-19-116` contradicts it**: replacing the binary lowers
layers 2 AND 3 without controlling the environment the TUI starts in at all,
through a command layer 2 permits — while `env -u GIT_CONFIG_COUNT` is REFUSED.
`19-27` read it and correctly declined because it was outside its
`files_modified`; **this round names it and `19-29` lists `src/envelope/mod.rs`.**

### Still OPEN and unchanged by this plan

- **`T-19-86`** — `high`, OPEN by explicit user scoping decision. Four registered
  rows at exit 0, pins green and UNMODIFIED, `T-19-111` kept OUT of it.
- **`T-19-91`** — `high`, OPEN, three arms unweakened.
- **`T-19-111`** — `high`, OPEN, **NO rule**, corpus rows RECORDED and asserted in
  neither direction. `19-27`'s five-site attribution correction stays as
  performed and is not moved back into `T-19-86`.
- **`T-19-112` / `T-19-113`** — NARROWED, not closed. Direction (i) is the residue
  they share with `T-19-118`, so this round's rows for it are theirs too.
- **`T-19-96`, `T-19-110`, `T-19-74`** — registered, not fixed; `T-19-74`'s core
  rows frozen.
- **`T-19-61` … `T-19-73`, `T-19-84`, `T-19-85`** — open and unaccepted by explicit
  user decision. No park-coverage controls added; `scan.rs` and `config.rs` not
  opened.
- **`C-08`** — mechanical half settled and unchanged; **behavioural half
  UNMEASURED and claimed in neither direction.** Repairing the delivery is a
  change to the SPAWN SEAM.
- **`C-15`** — a DISTINCT route to SAFE-06, steered by `HOME`/`XDG_CONFIG_HOME`
  and not by argv (`envelope guard` ignores the global `--config` flag and always
  reads `Config::default_path()`, `hooks.rs:837-840`). **No `pr_cap_*` clamp is
  written or proposed.**
- **the `glab --host` forge cell** — unchanged. **`FORGE_VALUE_OPTS` KEEPS
  `--hostname`** (audit 9 overturned audit 8's own suggestion: removal moves a
  counted creation form to UNCOUNTED, `T-19-35`). **`glab` is confirmed NOT
  INSTALLED**, so its latency cell is recorded as unmeasurable against its callee
  rather than driven or inferred, and no pin that would SKIP is written.
...
## Plan 19-32 — round 13's CORPUS (the rules are `19-33`'s)

**This plan closes nothing and clears no gate.** It measures both silent axes and
the scan's cost, widens the corpus so the axis can fail on both new classes,
observes RED, and stops. Whether `19-33`'s rules close anything is audit 13's
judgement rather than either plan's claim. **`/gsd-secure-phase 19` is NOT
cleared by this plan, by `19-33`, or by the two together.**

### THE RESIDUE, RESTATED AS A CONDITION OVER THREE AXES

Audit 12's diagnosis is adopted whole: `protected_carrier_named`'s residue
condition (`policy.rs:5900-5905`) **enumerates the silences of a READING**, and a
boundary has THREE ways to be silent. The condition names the word set and the
path set as PREMISES and then says nothing about either.

```text
AXIS 1  WHICH WORDS REACH THE READER      silence: a word the tokenizer produced
                                          for no class at all — an incomplete
                                          production, which FAILS CLOSED at
                                          `Token::redirection_unresolvable`
AXIS 2  WHAT THE READER SEES IN A WORD    silences: a word the SHELL MAY REWRITE;
                                          a word whose TEXT CARRIES NO ABSOLUTE
                                          PATH ANYWHERE; a word that reaches a
                                          protected path ONLY THROUGH A LINK
AXIS 3  WHICH PATHS THE COMPARISON        silences: an ancestor ABOVE the root —
        PROTECTS                          `$HOME`, `/tmp`, `/` — and the BINARY's
                                          PARENT and every ancestor of it
```
...
**The production docs still state the residue over ONE axis. `19-33` restates
them**, and that repair is registered here whether or not any rule lands.

### `T-19-122` — high, BLOCKING, OPEN. Corpus written and RED.

A NON-PATHNAME redirection operator's target is deleted from the token stream and
recorded on no `Segment` field, **so it is in NEITHER of the two word classes
`protected_carrier_named` iterates** — while being fully `Token.literal`,
ABSOLUTE at index zero, needing no cwd and no link, and naming the directory
exactly. None of the three declared silences.

* `xargs rm -rf <<< <ENV>/alpha` exit 0 against its twin `rm -rf <ENV>/alpha` at
  exit 2. Driven END TO END: a bare remote's `main` MOVED after a force push the
  hook had REFUSED, and a FIRED PR cap RESET in one persistent root.
* `xargs cp /bin/true <<< <BINARY>` exit 0 — **a NEW `T-19-116` route.**
* **`>&` with a path REACHES and was not predicted to.** Bash falls back to
  `&>word` when the operand is not an fd number, so `: >&<ledger>` truncates the
  ledger to zero bytes. A second operator, with a driven reach.
* **A CORRECTION TO AUDIT 12:** the heredoc BODY form is **already exit 2**,
  because `\n` is a `SEPARATORS` entry and the body is a segment of its own.
* **RECORDED, reach NOT demonstrated:** `<<` and `<<-` with the path as the
  DELIMITER, and `<&` with a path (`ambiguous redirect`).
* **PRE-EXISTING**: the rebuilt `dd17bfb` control binary permits it too.

**`19-33`'s design is MANDATED and DERIVED**: a SECOND `Segment` field carrying
LITERAL NON-PATHNAME target words, chained as a THIRD word class at the SAME
reading site. Widening `pathname_target` is FORBIDDEN — it turns
`only_a_literal_pathname_target_reaches_the_segment_and_the_tokens_do_not_move`
and the twelve-operator grammar pin RED, and the second field turns neither.

### `T-19-123` — high, BLOCKING, OPEN. Corpus written and RED.

`word_is_within` is a component-wise PREFIX over `<root>/<alias>` whose
`word.len() < dir.len()` early return answers `false` for every ANCESTOR.
`rm -rf <root>` takes the same nine carriers in ONE call, one component up.
`rm -rf <ENV>/`, `mv`, `find -delete`, `chmod 000` and `tar -C<ENV>` all reach,
and **`cd <ENV> && rm -rf alpha` defeats the narrowing
`protected_carrier_named`'s own doc claims** — the doc's own row is driven beside
it at exit 2. Driven END TO END twice, like `T-19-122`.

**It is PINNED PERMITTED BY NAME at `policy.rs:10022-10026` and
`tests/envelope_interior_path.rs:2341`, and the code does exactly what the pins
say.** This is a DECISION whose harm was never modelled, not a bug found.
**`19-33` moves both pins as a NAMED, BOUNDED, CROSS-FENCE exception with each
pin's reasoning REWRITTEN rather than deleted.** `19-32` performs neither move.

**The clause STOPS at `<root>`** on ownership and AR-19-11 grounds, and it is an
ANCESTOR clause and **not** the existing prefix widened to the root —
`ls <ENV>/unrelated-sibling` and the user-set-root case are pinned PERMITTED
before and after, so the wrong design turns the corpus red.

**RESIDUE, registered and UNACCEPTED: an ancestor ABOVE the root** —
`rm -rf /tmp` with `GSD_MM_ENVELOPE_ROOT=/tmp/xyz` — reaches the same nine
carriers and no rule is written for it.

### `T-19-124` — medium, OPEN. The property is written; the bound is `19-33`'s.

Round 12's own mechanism turned a linear scan QUADRATIC on the guard's critical
path: `slash_anchored_candidates` is applied PER CANDIDATE, so
`lexical_absolute_components` runs once per `/` over a suffix whose length is
itself O(word).

```text
 2 000 slashes    249 ms      dd17bfb:  5 ms
 8 000 slashes  3 904 ms      dd17bfb:  8 ms      ratio 15.7
 9 000 / 10 000 slashes: GUARD_TIMEOUT_SECS = 5 crossed between 18 KB and 20 KB
50 000 slashes 151 249 ms     dd17bfb: 28 ms
800 000 bytes: DID NOT ANSWER in 300 s
100 000 chars, NO slashes: 29 ms  <- the cost is the SCAN, not the LENGTH
```

`MAX_GUARD_REQUEST_BYTES = 1 MiB` bounds the INPUT and bounds **no work** —
`T-19-120`'s own shape in the round that fixed `T-19-120`.

**THE WORK-BOUND PROPERTY `19-33` MUST SATISFY: LINEARITY, with a FAIL-CLOSED
work ceiling beside it as the backstop.** Audit 12's parenthetical byte-floor
remedy is derived INSUFFICIENT: it removes NINE candidates of EIGHT THOUSAND for
the 16 KB word. A per-word cap is amplified by the word count and a per-segment
cap by the segment count.

**THE BEHAVIOURAL HALF — what the agent CLI does with a hook that overruns its
timeout — is UNMEASURED and claimed in NEITHER direction.**

### `T-19-125` — low, OPEN. The behaviour is asserted; the repairs are `19-33`'s.

The disclosed cost says *contains … as a `/`-anchored substring*; the comparison
NORMALISES, and `dd of=<ENV>/./alpha/x` and `dd of=<ENV>/zzz/../alpha/x` are both
exit 2 while containing no such substring at all. **Registered as `19-33`'s to
repair WHETHER OR NOT ANY RULE LANDS**, together with
`envelope_carrier_refusal`'s *"To proceed: name a path outside that directory"*,
which is STALE for exactly this case.

### `T-19-126` — low, OPEN. Two standing pins written; the repairs are `19-33`'s.

*(i)* `ledger.rs:354-361`'s forbidden-API gate is a **plan-time grep that nothing
re-runs**, and its stated reason is **INVERTED**. Two REAL standing pins now
exist — a BEHAVIOURAL one driving a symlinked-to-regular ledger PERMITTED and
COUNTED, and a SOURCE-SLICE one over `ledger.rs`'s production half that CAN name
the forbidden API because the name lives below the `#[cfg(test)]` sentinel.
**Both are GREEN before and after; they are standing controls, not evidence of a
fix.** The inverted comment is `19-33`'s to correct.

*(ii)* `CONTROL_CARRIER_INTERIOR_PATH` entry 7 does not reach the file under real
GNU tar. **Repaired by an ADDED reaching entry and an APPENDED WR-02 note; entry
7 stays and nothing is deleted or reworded.** Registered as `19-33`'s to carry
forward.

### `T-19-116` GAINED TWO ROUTES THIS AUDIT, AND CLOSING `T-19-122` CLOSES ONLY ONE

* `xargs cp /bin/true <<< <BINARY>` — **closed by the third word class.**
* `rm -rf <binary-parent>` — **NOT closed.** No ancestor clause is written for
  the binary and `word_is_exactly` stays an EQUALITY, because the binary's
  directory is shared with everything the user installed. **`word_is_exactly`'s
  reasoning argues only that a PREFIX would be wrong and never that the parent's
  own DELETION removes the binary — an HONESTY gap `19-33` repairs in that doc.**

The four original residues stay at exit 0 and `T-19-116` stays OPEN at `high`.

### Still OPEN and unchanged

- **`T-19-86`** — open at `high` by explicit user scoping decision. **The
  `!`-bodied alias credential route (`git -c alias.q='!git -c
  credential.helper=store credential fill' q`, exit 0 at the guard, secret
  PRESENT against real git with its no-`-c` control at exit 128 ABSENT) is
  `T-19-86`'s route and is RECORDED, never folded** into its declared harm and
  never used to re-rate it. Audit 12 declined to fold it and so does this plan.
  The missing `cred.rs` WHAT IT DOES NOT COVER list item is `19-33`'s to add.
- **`T-19-91`** — no rule, no remedy, not re-scoped.
- **`T-19-111`** — no rule; `19-27`'s five-site attribution correction stands and
  attribution stays OUT of `T-19-86`. Rows RECORDED in neither direction.
- **`T-19-112`** — narrowed further by BOTH of this round's classes and NOT
  closed; the cap was reset by a here-string, by `>&` and by the ancestor.
- **`T-19-113`**, **`T-19-115`** — no rule. `T-19-115` appears on axis 2 of the
  restated condition.
- **`T-19-121`** — narrowed HARD, not closed; a bounded residue of a control that
  genuinely landed.
- **`T-19-96`**, **`T-19-110`**, **`T-19-74`** (core rows frozen) — unchanged.
- **`T-19-84`**, **`T-19-85`**, **`T-19-61` … `T-19-73`** — open and unaccepted by
  explicit user decision; `scan.rs` and `config.rs` were not opened.
- **`C-08`** — its behavioural half stays **UNMEASURED** and is claimed in neither
  direction. `hooks.rs` was not opened.
- **`C-11` … `C-15`** — control (e), no rule and **no `pr_cap_*` clamp**.
- **The `glab --host` forge cell** — unfixed, `FORGE_VALUE_OPTS` keeps
  `--hostname`, and **`glab` is confirmed NOT INSTALLED**.
- **`T-19-104`'s `GIT_CONFIG_PARAMETERS` carrier** — a DIFFERENT mechanism,
  recorded with its overlap stated (its control fails the same way at the same
  identifier, so it discriminates nothing on these axes).
- **The two `tests/driver_reattach.rs` flakes** — one fired in the full-suite run
  and both on an isolated re-run. **RECORDED with verbatim output, not fixed, and
  their relation to this round claimed in NEITHER direction.** `envelope_tracer`
  answered 6/0 with no `ExecutableFileBusy` firing; **absence is not evidence
  either flake is fixed.**
- **`T-19-17r`** — OUTSTANDING for the **SEVENTEENTH** time. No `AR-19-13`, no
  Accepted-Risks-Log row, and the word "accepted" is applied to it nowhere.
  `AR-19-04` and `AR-19-05` are carried forward not un-accepted, not re-rated and
  not renumbered, and **`T-19-23` is NOT marked closed.**
