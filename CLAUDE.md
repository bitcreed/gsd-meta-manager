...
3. **Verify clean** after the bumps by running `./scripts/pre-tag-check.sh vX.Y.Z` with the tag you are about to create — it reproduces, locally and before the tag exists, the gates `.github/workflows/release.yml` runs on a tag push (tag/version match, MSRV floor, release build, tests, clippy). Every run also reconciles the locally installed git against `CONFIG_SECTION_CONSTANTS_DERIVED_AGAINST_GIT_VERSION` in `src/envelope/policy.rs`; a mismatch there is an advisory to read, not a script failure, because the GitHub `ubuntu-latest` runner's ambient git — not this machine's — is the authority for that witness test. So a green local run does not by itself guarantee the publish job passes, and while the advisory stands the version witness is expected to fail locally.
