...
|------------|---------|---------|-----------------|
| Rust | 1.88+ (stable) | Language | Zero-cost abstractions, single-binary distribution, no GC pauses, Cargo ecosystem |
...
3. **Verify clean with the container dry run** after the bumps: `./scripts/pre-tag-check.sh --container vX.Y.Z`, with the tag you are about to create. That mode builds `scripts/pre-tag-check.Dockerfile` (an `ubuntu-latest` lookalike), provisions the GSD conformance oracle inside it with the **same `scripts/install-conformance-oracle.sh` the publish job calls**, and re-runs the gates in there — so a green container run certifies the publish job in a way a bare local run cannot. It needs a usable container runtime (`docker`, else `podman`) and **fails loudly rather than falling back** to a local run; `PRE_TAG_CONTAINER_RUNTIME` forces a specific one.

   **Why a bare `./scripts/pre-tag-check.sh vX.Y.Z` cannot certify the publish job.** It reproduces the same five gates (tag/version match, MSRV floor, release build, tests, clippy), but on *this* machine — which has GSD installed and a different ambient git than the runner. Both of the tests that have actually blocked publication therefore pass locally on precisely the trees CI rejects: the router-conformance oracle test (`tests/driver_router_conformance.rs`, which fails by design when the oracle is absent, as it is on a bare runner) and the git-version witness. That is what container mode exists to close; run the bare mode only when no runtime is available, and treat its green as unproven.

   **The git reconciliation banner** is what the BARE mode tells you: every run reconciles the locally installed git against `CONFIG_SECTION_CONSTANTS_DERIVED_AGAINST_GIT_VERSION` in `src/envelope/policy.rs`. A mismatch is an advisory to read, not a script failure, because the runner's ambient git — not this machine's — is the authority for that witness test; while the advisory stands, the witness is expected to fail locally. In container mode the two git versions agree, so that banner should read **MATCH**, and a MISMATCH there means the Dockerfile's git has drifted from the constant and the image needs fixing.
...
6. **Annotated tag with the full changelog in the annotation body** — not just a one-line title. Example:
   ...
   Changes:
   - <quick-task id>  <short title>
   - <phase number>   <short title>

   Dependencies:
   - cargo update: <highlights>
