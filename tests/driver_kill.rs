...
/// process lifetime so the root outlives every test that drives a run — which
/// here includes the runs spawned as CHILD processes, since a child inherits
/// the environment this sets.
