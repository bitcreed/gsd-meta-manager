...
|---|---|---|---|---|
| `ttbook-ROADMAP.md` | ttbook | `9b7b305` | 2026-09-24 | `#### Build phase N (Milestone M): Title` placeholder phases 14-18 under `### 📋 Milestone M "…" (planned)` headings; build-phase dependency lines (`Build phases 8-13`, `Build phases 12, 13`, `Build phase 14`); the old milestone detector read each build heading as a spurious milestone; shipped v1 phases written as non-bold lines inside a `✅ … SHIPPED` `<details>` collapse (not GSD phases; read into `ProjectState::shipped_phases` since quick 260926-fi9) |
